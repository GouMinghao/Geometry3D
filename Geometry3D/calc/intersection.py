    ...
    **Output:**
    ...
    if a is None or b is None:
        get_main_logger().warning(
            "Intersecting something with None, which is always None"
        )
        return None
    # Point
    ...
    # halfline
        ...
        raise NotImplementedError(
            "not implement intersecting %s with %s" % (type(a), type(b))
        )
...
def inter_segment_convexpolyhedron(a, b):
    ...
    if len(inter_point_list) == 0:
        ...
        get_main_logger().error(
            "length of inter_point_list is {}, list is {}".format(
                len(inter_point_list), inter_point_list
            )
        )
        raise TypeError("Bug detected! please contact the author")
...
def inter_convexpolygon_convexpolygon(a, b):
    ...
    if inter_p_p is None:
        ...
        for seg in a.segments():
            point_set = point_set.union(
                get_segment_convexpolygon_intersection_point_set(seg, b)
            )
        point_tuple = tuple(point_set)
        if len(point_tuple) == 0:
            return None
...
def inter_convexpolygon_convexPolyhedron(cph, cpg):
    ...
    if inter_p_cph is None:
        ...
        isinstance(inter_p_cph, Point)
        or isinstance(inter_p_cph, Segment)
        or isinstance(inter_p_cph, ConvexPolygon)
    ):
...
def inter_convexpolyhedron_convexpolyhedron(cph1, cph2):
    ...
    if not (isinstance(cph1, ConvexPolyhedron) and isinstance(cph2, ConvexPolyhedron)):
        raise ValueError(
            "convex polyhedron intersection should be called with two ConvexPolyhedre"
        )
    cpg_set = set()
    segment_set = set()
    point_set = set()
