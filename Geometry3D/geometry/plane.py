...
class Plane(GeoBody):
    ...
    def __init__(self, *args):
        if len(args) == 3:
            a, b, c = args
            if isinstance(a, Point) and isinstance(b, Point) and isinstance(b, Point):
                ...
                isinstance(a, Point) and isinstance(b, Vector) and isinstance(c, Vector)
            ):
                vab, vac = b, c
    ...
    def __hash__(self):
        ...
        return hash(
            (
                "Plane",
                round(self.n[0], SIG_FIGURES),
                round(self.n[1], SIG_FIGURES),
                round(self.n[2], SIG_FIGURES),
                round(self.n * self.p.pv(), SIG_FIGURES),
            )
        )
