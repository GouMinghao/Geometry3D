...
def get_circle_point_list(center, normal, radius, n=10):
    if n <= 2:
        raise ValueError(
            "n must be at least 3 to construct an inscribed polygon for a circle"
        )
    import math, copy
    ...
    for i in range(n):
        ...
        point_list.append(
            copy.deepcopy(center).move(v1 * math.cos(angle_i) + v2 * math.sin(angle_i))
        )
    return point_list
    ...
    def _get_center_point(self):
        ...
        **Output:**
        ...
        for point in self.points:
            ...
            z += point.z
        return Point(
            float(x) / num_points, float(y) / num_points, float(z) / num_points
        )
        ...
        **Output:**
        ...
        for i in range(len(self.points)):
            ...
            area += get_triangle_area(
                self.center_point, self.points[index_0], self.points[index_1]
            )
        return area
        ...
        **Output:**
        ...
        for point in self.points:
            if not point in self.plane:
                raise ValueError(
                    "Convex Check Fails Because {} Is Not On {}".format(
                        point, self.plane
                    )
                )
