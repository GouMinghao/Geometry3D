    ...
    **Input:**
    ...
    def __init__(self, a, b):
        ...
        if isinstance(a, Point) and isinstance(b, Point):
            if a == b:
                raise ValueError(
                    "Cannot initialize a HalfLine with two identical Points"
                )
            self.line = Line(a, b)
            self.point = a
            ...
            if b.length() < get_eps():
                raise ValueError(
                    "Cannot initialize a HalfLine with the length of Vector is 0"
                )
            self.line = Line(a, b)
            self.point = a
            self.vector = b
            ...
            raise ValueError(
                "Cannot create segment with type:%s and %s" % (type(a), type(b))
            )

    def __eq__(self, other):
        return (
            self.point == other.point
            and (self.vector.normalized() - other.vector.normalized()).length()
            < get_eps()
        )
    ...
    def __contains__(self, other):
        ...
        if isinstance(other, HalfLine):
            return (
                ...
                and (other.point in self)
                and ((self.vector * other.vector) > -get_eps())
            )
        else:
            get_main_logger().warning(
                "Calling type {} in type {} which is always False".format(
