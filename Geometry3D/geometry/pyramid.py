    ...
    def __init__(self, cp, p, direct_call=True):
        if direct_call:
            get_main_logger().warning(
                "Pyramid is an auxilary geometry. Direct use is not suggested. Consider using ConvexPolyhedron instead."
            )
        if isinstance(cp, ConvexPolygon) and isinstance(p, Point):
            ...
            if self.point in self.convex_polygon.plane:
                raise ValueError(
                    "Cannot create Pyramid with point on the polygon plane"
                )
        else:
            raise ValueError(
                "Cannot create Pyramid with type:%s and %s" % (type(cp), type(p))
            )
